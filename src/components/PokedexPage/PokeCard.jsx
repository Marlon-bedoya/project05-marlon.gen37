...
const PokeCard = ({ pokeInfo }) => {
    ...
    return (
        <article className={`card br-${pokemon?.types[0].type.name}`} onClick={handlePokeDetail}>
                ...
                <ul className="card__list" >
                    {
                        pokemon?.stats.map(statInfo => (
                            <li className="card__stat" key={statInfo.stat.url}>
                                ...
                                <span className={`card__stat__valu color-${pokemon?.types[0].type.name}`} >{statInfo.base_stat}</span>
                            </li>
                        ))
                    }
                </ul>
